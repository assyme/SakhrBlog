 ...
 */
  ...
  $.fn.keynav = function (onClass,offClass) {
	  ...
	  if(!kn.init) {
		  ...
		  $(document).bind("keydown", kn.keyDown);
		  kn.init = true;
	  }
  ...
  $.keynav.reset = function() {
	  ...
	  if (kn.el != undefined) {
 	  for(var i=0;i<kn.el.length;i++) {
  		 kn.el[i].onmouseover = null;
	   }
	  }
	  kn.el = new Array();
	  $(document).unbind("keydown", kn.keyDown);
	  kn.init = false;
  };
...
//	  }
   if (kn.currentEl != undefined)
   {
    var tmp = kn.currentEl;
    $(tmp).removeClass(tmp.onClass).addClass(tmp.offClass);
   }
...
//		nd = Math.sqrt(Math.pow(cur.pos.cx-e.pos.cx,2)+Math.pow(cur.pos.cy-e.pos.cy,2));
		nd = Math.abs(cur.pos.cx-e.pos.cx)+Math.abs(cur.pos.cy-e.pos.cy);
		if(nd < od) {
			closest = e;
			od = nd;
			found = true;
